...
poppler_path = r"" # Enter your poppler path upto the "bin" folder
...
def call_LLM_for_invoice_split(system_prompt, image_data_url):
    ...
    llm_with_image_context = llm.bind(images=[image_data_url])

    response = llm_with_image_context.invoke([
        SystemMessage(content=system_prompt),
        HumanMessage(content="This is the image passed to you")
    ])
...
def call_LLM_for_vendor_name_validation(system_prompt, vendor_name_1, vendor_name_2):
    ...
    response = llm.invoke([
        SystemMessage(content=validate_vendor_names_system_prompt),
...
# Function to validate customer names using the LLM
def call_LLM_for_customer_name_validation(system_prompt, customer_name_1, customer_name_2):
    ...
    response = llm.invoke([
        SystemMessage(content=validate_person_names_system_prompt),
        HumanMessage(content=user_query)
    ])
...
def process_invoice_classification():
    ...
    for i, invoice in enumerate(invoices):
        ...
        for page_num in range(len(reader.pages)):
            ...
            previous_page_classification = call_LLM_for_invoice_classification(system_prompt_for_invoice_classification, base64_url)

            if page_num == len(reader.pages) - 1:
                # If it's the last page, add the classification to the final_output dictionary
                final_output[invoice] = previous_page_classification.strip()
                print(f"Final classification for {invoice}: {previous_page_classification.strip()}")
...
if uploaded_file:
    ...
    if not os.path.exists("output_files") or len(os.listdir("output_files")) == 0:
        st.info("Processing pages with Local LLM...")
